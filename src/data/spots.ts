export type SpotCategory =
  | 'hill'
  | 'beach'
  | 'coastal-bluff'
  | 'park'
  | 'skyscraper'
  | 'waterfront';

export type AccessAlertType = 'hike' | 'tide' | 'hours' | 'paid' | 'info';

export type City = 'sf' | 'austin' | 'santa-cruz';

// Optional, sparingly applied. Only attach when there's something a visitor
// genuinely needs to know before going (gate hours, tide windows, hike
// requirements, paid venues). Most spots intentionally have no alert.
export interface AccessAlert {
  message: string;
  type: AccessAlertType;
}

export interface Spot {
  id: string;
  name: string;
  lat: number;
  lng: number;
  city: City;
  category: SpotCategory;
  elevation: number; // meters
  lightPollution: 'Low' | 'Mid' | 'High';
  horizonQuality: 'Open' | 'Partial' | 'Blocked';
  sunrise: number;   // 0-100
  sunset: number;    // 0-100
  stargazing: number; // 0-100
  accessAlert?: AccessAlert;
}

export const spots: Spot[] = [
  {
    id: "sf-ocean-beach", name: "Ocean Beach",
    lat: 37.7594, lng: -122.5107,
    city: 'sf',
    category: 'beach', elevation: 3,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 25, sunset: 95, stargazing: 55,
  },
  {
    id: "sf-lands-end", name: "Lands End / Sutro Baths",
    lat: 37.7803, lng: -122.5115,
    city: 'sf',
    category: 'coastal-bluff', elevation: 60,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 35, sunset: 92, stargazing: 60,
  },
  {
    id: "sf-fort-funston", name: "Fort Funston",
    lat: 37.7148, lng: -122.5026,
    city: 'sf',
    category: 'coastal-bluff', elevation: 60,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 20, sunset: 90, stargazing: 65,
  },
  {
    id: "sf-hippie-hill", name: "Golden Gate Park — Hippie Hill",
    lat: 37.7694, lng: -122.4530,
    city: 'sf',
    category: 'park', elevation: 30,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 40, sunset: 50, stargazing: 30,
  },
  {
    id: "sf-strawberry-hill", name: "Golden Gate Park — Strawberry Hill",
    lat: 37.7690, lng: -122.4758,
    city: 'sf',
    category: 'park', elevation: 128,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 55, sunset: 65, stargazing: 40,
  },
  {
    id: "sf-sutro-heights", name: "Sutro Heights Park",
    lat: 37.7782, lng: -122.5105,
    city: 'sf',
    category: 'hill', elevation: 61,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 30, sunset: 88, stargazing: 58,
  },
  {
    id: "sf-twin-peaks", name: "Twin Peaks",
    lat: 37.7544, lng: -122.4477,
    city: 'sf',
    category: 'hill', elevation: 282,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 90, sunset: 92, stargazing: 72,
  },
  {
    id: "sf-corona-heights", name: "Corona Heights Park",
    lat: 37.7654, lng: -122.4390,
    city: 'sf',
    category: 'hill', elevation: 155,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 70, sunset: 75, stargazing: 48,
  },
  {
    id: "sf-buena-vista", name: "Buena Vista Park",
    lat: 37.7688, lng: -122.4418,
    city: 'sf',
    category: 'park', elevation: 175,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 65, sunset: 70, stargazing: 42,
  },
  {
    id: "sf-dolores-park", name: "Dolores Park",
    lat: 37.7596, lng: -122.4269,
    city: 'sf',
    category: 'park', elevation: 61,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 45, sunset: 72, stargazing: 20,
  },
  {
    id: "sf-grandview-park", name: "Grandview Park",
    lat: 37.7557, lng: -122.4710,
    city: 'sf',
    category: 'hill', elevation: 227,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 78, sunset: 85, stargazing: 55,
  },
  {
    id: "sf-bernal-heights", name: "Bernal Heights Park",
    lat: 37.7432, lng: -122.4155,
    city: 'sf',
    category: 'hill', elevation: 130,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 82, sunset: 80, stargazing: 50,
  },
  {
    id: "sf-crissy-field", name: "Crissy Field",
    lat: 37.8039, lng: -122.4650,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 55, sunset: 85, stargazing: 35,
  },
  {
    id: "sf-marina-green", name: "Marina Green",
    lat: 37.8065, lng: -122.4370,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 50, sunset: 78, stargazing: 28,
  },
  {
    id: "sf-fort-mason", name: "Fort Mason Great Meadow",
    lat: 37.8050, lng: -122.4310,
    city: 'sf',
    category: 'park', elevation: 20,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 55, sunset: 74, stargazing: 25,
  },
  {
    id: "sf-huntington-park", name: "Huntington Park",
    lat: 37.7923, lng: -122.4120,
    city: 'sf',
    category: 'park', elevation: 107,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 35, sunset: 40, stargazing: 12,
  },
  {
    id: "sf-ina-coolbrith", name: "Ina Coolbrith Park",
    lat: 37.7983, lng: -122.4140,
    city: 'sf',
    category: 'hill', elevation: 91,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 55, sunset: 60, stargazing: 18,
  },
  {
    id: "sf-coit-tower", name: "Coit Tower / Pioneer Park",
    lat: 37.8024, lng: -122.4058,
    city: 'sf',
    category: 'hill', elevation: 82,
    lightPollution: 'High', horizonQuality: 'Open',
    sunrise: 75, sunset: 68, stargazing: 22,
  },
  // FLAG: id 19 (Washington Square Park) and id 48 (Joe DiMaggio Playground)
  // sit within ~0.001° of each other. The playground is technically inside
  // the same block as the square — kept separate for now because they have
  // distinct entrances and use cases. Revisit if we ever consolidate.
  {
    id: "sf-washington-square", name: "Washington Square Park",
    lat: 37.8009, lng: -122.4103,
    city: 'sf',
    category: 'park', elevation: 12,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 20, sunset: 30, stargazing: 8,
  },
  {
    id: "sf-portsmouth-square", name: "Portsmouth Square",
    lat: 37.7953, lng: -122.4050,
    city: 'sf',
    category: 'park', elevation: 25,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 15, sunset: 22, stargazing: 5,
  },
  {
    id: "sf-alta-plaza", name: "Alta Plaza Park",
    lat: 37.7910, lng: -122.4380,
    city: 'sf',
    category: 'park', elevation: 72,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 55, sunset: 62, stargazing: 25,
  },
  {
    id: "sf-lafayette-park", name: "Lafayette Park",
    lat: 37.7918, lng: -122.4285,
    city: 'sf',
    category: 'park', elevation: 107,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 50, sunset: 58, stargazing: 22,
  },
  {
    id: "sf-mclaren-park", name: "McLaren Park",
    lat: 37.7190, lng: -122.4200,
    city: 'sf',
    category: 'park', elevation: 155,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 60, sunset: 65, stargazing: 62,
  },
  {
    id: "sf-india-basin", name: "India Basin Shoreline Park",
    lat: 37.7370, lng: -122.3780,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 78, sunset: 45, stargazing: 32,
  },
  {
    id: "sf-rincon-park", name: "Rincon Park",
    lat: 37.7906, lng: -122.3883,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 68, sunset: 55, stargazing: 10,
  },
  // — Additional SF Parks —
  {
    id: "sf-glen-canyon", name: "Glen Canyon Park",
    lat: 37.7410, lng: -122.4430,
    city: 'sf',
    category: 'park', elevation: 90,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 50, sunset: 55, stargazing: 45,
  },
  {
    id: "sf-mount-davidson", name: "Mount Davidson Park",
    lat: 37.7380, lng: -122.4545,
    city: 'sf',
    category: 'hill', elevation: 282,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 72, sunset: 78, stargazing: 68,
  },
  // FLAG: id 28 (Stern Grove) and id 74 (Sigmund Stern Recreation Grove) are
  // ~0.002° apart and arguably the same park. Kept separate because the
  // existing entries cover slightly different sub-areas (meadow vs. rec
  // grove) — review if/when we add proper sub-area metadata.
  {
    id: "sf-stern-grove", name: "Stern Grove",
    lat: 37.7370, lng: -122.4710,
    city: 'sf',
    category: 'park', elevation: 30,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 20, sunset: 25, stargazing: 30,
  },
  {
    id: "sf-pine-lake", name: "Pine Lake Park",
    lat: 37.7375, lng: -122.4780,
    city: 'sf',
    category: 'park', elevation: 40,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 30, sunset: 35, stargazing: 35,
  },
  {
    id: "sf-balboa-park", name: "Balboa Park",
    lat: 37.7215, lng: -122.4415,
    city: 'sf',
    category: 'park', elevation: 20,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 18, sunset: 22, stargazing: 15,
  },
  {
    id: "sf-crocker-amazon", name: "Crocker Amazon Park",
    lat: 37.7105, lng: -122.4335,
    city: 'sf',
    category: 'park', elevation: 30,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 30, sunset: 35, stargazing: 25,
  },
  {
    id: "sf-philosophers-way", name: "John McLaren Park — Philosopher's Way",
    lat: 37.7185, lng: -122.4145,
    city: 'sf',
    category: 'park', elevation: 155,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 65, sunset: 70, stargazing: 60,
  },
  {
    id: "sf-bayview-park", name: "Bayview Park",
    lat: 37.7305, lng: -122.3920,
    city: 'sf',
    category: 'hill', elevation: 90,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 72, sunset: 65, stargazing: 38,
  },
  {
    id: "sf-herons-head", name: "Heron's Head Park",
    lat: 37.7385, lng: -122.3725,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 75, sunset: 40, stargazing: 28,
  },
  {
    id: "sf-candlestick-point", name: "Candlestick Point SRA",
    lat: 37.7130, lng: -122.3830,
    city: 'sf',
    category: 'waterfront', elevation: 5,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 70, sunset: 50, stargazing: 30,
  },
  {
    id: "sf-potrero-hill", name: "Potrero Hill Mini Park",
    lat: 37.7605, lng: -122.3990,
    city: 'sf',
    category: 'hill', elevation: 90,
    lightPollution: 'High', horizonQuality: 'Open',
    sunrise: 72, sunset: 70, stargazing: 18,
  },
  {
    id: "sf-mckinley-square", name: "McKinley Square Park",
    lat: 37.7590, lng: -122.4040,
    city: 'sf',
    category: 'hill', elevation: 60,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 55, sunset: 52, stargazing: 15,
  },
  {
    id: "sf-holly-park", name: "Holly Park",
    lat: 37.7415, lng: -122.4260,
    city: 'sf',
    category: 'hill', elevation: 85,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 68, sunset: 72, stargazing: 35,
  },
  {
    id: "sf-precita-park", name: "Precita Park",
    lat: 37.7475, lng: -122.4120,
    city: 'sf',
    category: 'park', elevation: 30,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 20, sunset: 25, stargazing: 10,
  },
  {
    id: "sf-st-marys-park", name: "St. Mary's Park",
    lat: 37.7365, lng: -122.4200,
    city: 'sf',
    category: 'park', elevation: 55,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 40, sunset: 45, stargazing: 28,
  },
  {
    id: "sf-garfield-square", name: "Garfield Square",
    lat: 37.7560, lng: -122.4130,
    city: 'sf',
    category: 'park', elevation: 20,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 15, sunset: 18, stargazing: 8,
  },
  {
    id: "sf-jackson-park", name: "Jackson Park",
    lat: 37.7522, lng: -122.3975,
    city: 'sf',
    category: 'park', elevation: 15,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 18, sunset: 20, stargazing: 8,
  },
  {
    id: "sf-esprit-park", name: "Esprit Park",
    lat: 37.7618, lng: -122.3905,
    city: 'sf',
    category: 'park', elevation: 5,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 22, sunset: 25, stargazing: 5,
  },
  {
    id: "sf-south-park", name: "South Park",
    lat: 37.7825, lng: -122.3940,
    city: 'sf',
    category: 'park', elevation: 5,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 18, sunset: 20, stargazing: 5,
  },
  {
    id: "sf-yerba-buena", name: "Yerba Buena Gardens",
    lat: 37.7855, lng: -122.4025,
    city: 'sf',
    category: 'park', elevation: 10,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 12, sunset: 15, stargazing: 3,
  },
  {
    id: "sf-sue-bierman", name: "Sue Bierman Park",
    lat: 37.7960, lng: -122.3970,
    city: 'sf',
    category: 'park', elevation: 5,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 60, sunset: 40, stargazing: 5,
  },
  {
    id: "sf-sidney-walton", name: "Sidney Walton Park",
    lat: 37.7985, lng: -122.3990,
    city: 'sf',
    category: 'park', elevation: 5,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 15, sunset: 18, stargazing: 3,
  },
  {
    id: "sf-joe-dimaggio", name: "Joe DiMaggio Playground",
    lat: 37.8010, lng: -122.4105,
    city: 'sf',
    category: 'park', elevation: 15,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 15, sunset: 18, stargazing: 5,
  },
  // id 49 ("Pioneer Park (Telegraph Hill)") removed — exact duplicate of
  // id 18 ("Coit Tower / Pioneer Park"). IDs are stable identifiers, so 49
  // is intentionally left as a gap rather than reused.
  {
    id: "sf-michelangelo", name: "Michelangelo Playground",
    lat: 37.8000, lng: -122.4155,
    city: 'sf',
    category: 'park', elevation: 25,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 15, sunset: 18, stargazing: 5,
  },
  {
    id: "sf-moscone-park", name: "Moscone Park",
    lat: 37.8005, lng: -122.4300,
    city: 'sf',
    category: 'park', elevation: 10,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 35, sunset: 45, stargazing: 15,
  },
  {
    id: "sf-george-sterling", name: "George Sterling Park",
    lat: 37.7965, lng: -122.4175,
    city: 'sf',
    category: 'park', elevation: 75,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 45, sunset: 50, stargazing: 15,
  },
  {
    id: "sf-allyne-park", name: "Allyne Park",
    lat: 37.7960, lng: -122.4230,
    city: 'sf',
    category: 'park', elevation: 55,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 40, sunset: 45, stargazing: 18,
  },
  {
    id: "sf-lyon-street-steps", name: "Pacific Heights Park (Lyon Street Steps)",
    lat: 37.7945, lng: -122.4470,
    city: 'sf',
    category: 'park', elevation: 60,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 50, sunset: 55, stargazing: 20,
  },
  {
    id: "sf-presidio-main-post", name: "Presidio Main Post",
    lat: 37.7990, lng: -122.4580,
    city: 'sf',
    category: 'park', elevation: 35,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 45, sunset: 60, stargazing: 48,
  },
  {
    id: "sf-inspiration-point", name: "Presidio — Inspiration Point",
    lat: 37.7930, lng: -122.4650,
    city: 'sf',
    category: 'hill', elevation: 120,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 70, sunset: 82, stargazing: 55,
  },
  {
    id: "sf-lovers-lane", name: "Presidio — Lovers' Lane",
    lat: 37.7960, lng: -122.4615,
    city: 'sf',
    category: 'park', elevation: 50,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 35, sunset: 45, stargazing: 45,
  },
  {
    id: "sf-baker-beach", name: "Baker Beach",
    lat: 37.7935, lng: -122.4835,
    city: 'sf',
    category: 'beach', elevation: 5,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 30, sunset: 90, stargazing: 52,
  },
  {
    id: "sf-mountain-lake", name: "Mountain Lake Park",
    lat: 37.7880, lng: -122.4700,
    city: 'sf',
    category: 'park', elevation: 40,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 30, sunset: 40, stargazing: 30,
  },
  {
    id: "sf-rossi-playground", name: "Rossi Playground & Park",
    lat: 37.7825, lng: -122.4530,
    city: 'sf',
    category: 'park', elevation: 40,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 18, sunset: 20, stargazing: 12,
  },
  {
    id: "sf-panhandle-park", name: "Panhandle Park",
    lat: 37.7720, lng: -122.4405,
    city: 'sf',
    category: 'park', elevation: 30,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 20, sunset: 25, stargazing: 12,
  },
  {
    id: "sf-alamo-square", name: "Alamo Square Park",
    lat: 37.7762, lng: -122.4350,
    city: 'sf',
    category: 'park', elevation: 65,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 55, sunset: 62, stargazing: 22,
  },
  {
    id: "sf-jefferson-square", name: "Jefferson Square Park",
    lat: 37.7815, lng: -122.4270,
    city: 'sf',
    category: 'park', elevation: 20,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 15, sunset: 18, stargazing: 8,
  },
  {
    id: "sf-hayes-valley", name: "Hayes Valley Playground",
    lat: 37.7755, lng: -122.4290,
    city: 'sf',
    category: 'park', elevation: 20,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 15, sunset: 18, stargazing: 8,
  },
  {
    id: "sf-duboce-park", name: "Duboce Park",
    lat: 37.7695, lng: -122.4335,
    city: 'sf',
    category: 'park', elevation: 35,
    lightPollution: 'High', horizonQuality: 'Blocked',
    sunrise: 18, sunset: 22, stargazing: 10,
  },
  {
    id: "sf-dolores-upper", name: "Mission Dolores Park — Upper Terraces",
    lat: 37.7610, lng: -122.4270,
    city: 'sf',
    category: 'park', elevation: 80,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 55, sunset: 68, stargazing: 18,
  },
  {
    id: "sf-noe-valley-courts", name: "Noe Valley Courts",
    lat: 37.7502, lng: -122.4320,
    city: 'sf',
    category: 'park', elevation: 90,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 45, sunset: 50, stargazing: 22,
  },
  {
    id: "sf-billy-goat-hill", name: "Billy Goat Hill Park",
    lat: 37.7440, lng: -122.4370,
    city: 'sf',
    category: 'hill', elevation: 115,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 72, sunset: 75, stargazing: 42,
  },
  {
    id: "sf-walter-haas", name: "Walter Haas Playground",
    lat: 37.7395, lng: -122.4440,
    city: 'sf',
    category: 'park', elevation: 70,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 35, sunset: 40, stargazing: 28,
  },
  {
    id: "sf-sunnyside", name: "Sunnyside Playground",
    lat: 37.7325, lng: -122.4490,
    city: 'sf',
    category: 'park', elevation: 50,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 20, sunset: 22, stargazing: 18,
  },
  {
    id: "sf-miraloma-park", name: "Miraloma Park",
    lat: 37.7375, lng: -122.4560,
    city: 'sf',
    category: 'park', elevation: 140,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 55, sunset: 60, stargazing: 40,
  },
  {
    id: "sf-west-portal", name: "West Portal Playground",
    lat: 37.7400, lng: -122.4660,
    city: 'sf',
    category: 'park', elevation: 55,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 18, sunset: 20, stargazing: 15,
  },
  {
    id: "sf-larsen-park", name: "Larsen Park",
    lat: 37.7465, lng: -122.4870,
    city: 'sf',
    category: 'park', elevation: 45,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 22, sunset: 28, stargazing: 18,
  },
  {
    id: "sf-sigmund-stern", name: "Sigmund Stern Recreation Grove",
    lat: 37.7350, lng: -122.4730,
    city: 'sf',
    category: 'park', elevation: 45,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 18, sunset: 22, stargazing: 20,
  },
  {
    id: "sf-lake-merced", name: "Lake Merced Park",
    lat: 37.7275, lng: -122.4930,
    city: 'sf',
    category: 'waterfront', elevation: 5,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 40, sunset: 55, stargazing: 55,
  },
  {
    id: "sf-brotherhood-park", name: "Brotherhood Mini Park",
    lat: 37.7265, lng: -122.4525,
    city: 'sf',
    category: 'park', elevation: 70,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 30, sunset: 35, stargazing: 22,
  },
  {
    id: "sf-visitacion-valley", name: "Visitacion Valley Greenway",
    lat: 37.7160, lng: -122.4075,
    city: 'sf',
    category: 'park', elevation: 25,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 18, sunset: 20, stargazing: 12,
  },
  {
    id: "sf-cayuga-park", name: "Cayuga Park",
    lat: 37.7230, lng: -122.4395,
    city: 'sf',
    category: 'park', elevation: 30,
    lightPollution: 'Mid', horizonQuality: 'Blocked',
    sunrise: 15, sunset: 18, stargazing: 12,
  },
  {
    id: "sf-spreckels-lake", name: "Golden Gate Park — Spreckels Lake",
    lat: 37.7726, lng: -122.4865,
    city: 'sf',
    category: 'park', elevation: 25,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 30, sunset: 40, stargazing: 28,
  },
  {
    id: "sf-prayer-book-cross", name: "Golden Gate Park — Prayer Book Cross",
    lat: 37.7720, lng: -122.4615,
    city: 'sf',
    category: 'park', elevation: 55,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 42, sunset: 50, stargazing: 32,
  },
  {
    id: "sf-tank-hill", name: "Tank Hill Park",
    lat: 37.7610, lng: -122.4480,
    city: 'sf',
    category: 'hill', elevation: 200,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 82, sunset: 85, stargazing: 52,
  },
  {
    id: "sf-kite-hill", name: "Kite Hill Open Space",
    lat: 37.7575, lng: -122.4415,
    city: 'sf',
    category: 'hill', elevation: 220,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 80, sunset: 82, stargazing: 48,
  },
  {
    id: "sf-hawk-hill", name: "Hawk Hill (Marin Headlands)",
    lat: 37.8260, lng: -122.4990,
    city: 'sf',
    category: 'hill', elevation: 280,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 85, sunset: 95, stargazing: 78,
  },
  {
    id: "sf-aquatic-park", name: "Aquatic Park",
    lat: 37.8067, lng: -122.4225,
    city: 'sf',
    category: 'beach', elevation: 2,
    lightPollution: 'High', horizonQuality: 'Open',
    sunrise: 55, sunset: 70, stargazing: 12,
  },
  {
    id: "sf-crane-cove", name: "Crane Cove Park",
    lat: 37.7625, lng: -122.3825,
    city: 'sf',
    category: 'beach', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 72, sunset: 48, stargazing: 18,
  },
  {
    id: "sf-warm-water-cove", name: "Warm Water Cove Park",
    lat: 37.7570, lng: -122.3850,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 68, sunset: 45, stargazing: 15,
  },
  {
    id: "sf-islais-creek", name: "Islais Creek Promenade",
    lat: 37.7480, lng: -122.3810,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 70, sunset: 42, stargazing: 18,
  },
  // — Regional spots beyond central SF —
  // Travel times computed from straight-line distance are very rough for
  // these; treat anything > ~15 mi from downtown as a day trip. Hawk Hill
  // (id 83) already covers the Marin Headlands ridge — do not re-add.
  // — Marin / North —
  {
    id: "sf-mt-tam-east-peak", name: "Mount Tamalpais — East Peak",
    lat: 37.9235, lng: -122.5965,
    city: 'sf',
    category: 'hill', elevation: 784,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 88, sunset: 96, stargazing: 92,
  },
  {
    id: "sf-battery-spencer", name: "Battery Spencer",
    lat: 37.8320, lng: -122.4830,
    city: 'sf',
    category: 'hill', elevation: 75,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 70, sunset: 92, stargazing: 38,
  },
  {
    id: "sf-muir-beach-overlook", name: "Muir Beach Overlook",
    lat: 37.8595, lng: -122.5720,
    city: 'sf',
    category: 'coastal-bluff', elevation: 46,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 35, sunset: 92, stargazing: 78,
  },
  {
    id: "sf-muir-beach", name: "Muir Beach",
    lat: 37.8590, lng: -122.5800,
    city: 'sf',
    category: 'beach', elevation: 3,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 30, sunset: 88, stargazing: 72,
  },
  {
    id: "sf-bolinas-ridge", name: "Bolinas Ridge",
    lat: 37.9380, lng: -122.6600,
    city: 'sf',
    category: 'hill', elevation: 460,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 80, sunset: 92, stargazing: 90,
    accessAlert: {
      message: "Road closes 30 min after sunset. Get there early. Winding drive from Fairfax, ~45 min from SF.",
      type: 'hours',
    },
  },
  {
    id: "sf-rodeo-beach", name: "Rodeo Beach",
    lat: 37.8340, lng: -122.5400,
    city: 'sf',
    category: 'beach', elevation: 3,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 30, sunset: 82, stargazing: 70,
  },
  {
    id: "sf-point-reyes", name: "Point Reyes — Chimney Rock",
    lat: 38.0020, lng: -122.9640,
    city: 'sf',
    category: 'coastal-bluff', elevation: 30,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 60, sunset: 90, stargazing: 96,
  },
  {
    id: "sf-stinson-beach", name: "Stinson Beach",
    lat: 37.8970, lng: -122.6400,
    city: 'sf',
    category: 'beach', elevation: 3,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 32, sunset: 90, stargazing: 72,
  },
  // — East Bay —
  {
    id: "sf-grizzly-peak", name: "Grizzly Peak",
    lat: 37.8800, lng: -122.2430,
    city: 'sf',
    category: 'hill', elevation: 550,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 75, sunset: 94, stargazing: 55,
  },
  {
    id: "sf-treasure-island", name: "Treasure Island Viewpoint",
    lat: 37.8197, lng: -122.3700,
    city: 'sf',
    category: 'waterfront', elevation: 5,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 82, sunset: 88, stargazing: 30,
  },
  {
    id: "sf-lake-merritt", name: "Lake Merritt",
    lat: 37.8022, lng: -122.2600,
    city: 'sf',
    category: 'park', elevation: 2,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 38, sunset: 70, stargazing: 8,
  },
  {
    id: "sf-sibley-preserve", name: "Sibley Volcanic Regional Preserve",
    lat: 37.8750, lng: -122.1960,
    city: 'sf',
    category: 'park', elevation: 480,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 65, sunset: 70, stargazing: 75,
  },
  {
    id: "sf-mount-diablo", name: "Mount Diablo — Summit",
    lat: 37.8816, lng: -121.9142,
    city: 'sf',
    category: 'hill', elevation: 1173,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 92, sunset: 92, stargazing: 98,
  },
  // — Berkeley —
  {
    id: "sf-indian-rock", name: "Indian Rock Park",
    lat: 37.8920, lng: -122.2730,
    city: 'sf',
    category: 'hill', elevation: 100,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 65, sunset: 88, stargazing: 30,
  },
  {
    id: "sf-berkeley-rose-garden", name: "Berkeley Rose Garden",
    lat: 37.8860, lng: -122.2630,
    city: 'sf',
    category: 'park', elevation: 65,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 40, sunset: 75, stargazing: 20,
  },
  {
    id: "sf-cesar-chavez-park", name: "Cesar Chavez Park",
    lat: 37.8700, lng: -122.3170,
    city: 'sf',
    category: 'waterfront', elevation: 5,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 55, sunset: 90, stargazing: 25,
  },
  {
    id: "sf-big-c-trail", name: "The Big C Trail",
    lat: 37.8745, lng: -122.2480,
    city: 'sf',
    category: 'hill', elevation: 260,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 72, sunset: 82, stargazing: 35,
    accessAlert: { message: "Short but very steep hike from behind Bowles Hall on campus. ~15 minutes up. Wear real shoes.", type: 'hike' },
  },
  {
    id: "sf-wildcat-peak", name: "Tilden Regional Park — Wildcat Peak",
    lat: 37.9060, lng: -122.2430,
    city: 'sf',
    category: 'hill', elevation: 400,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 80, sunset: 85, stargazing: 60,
    accessAlert: { message: "~1.5 mile hike from the Environmental Education Center. Moderate climb. Park closes at 10 PM.", type: 'hike' },
  },
  {
    id: "sf-clark-kerr-fire-trail", name: "Clark Kerr Fire Trail",
    lat: 37.8650, lng: -122.2450,
    city: 'sf',
    category: 'hill', elevation: 320,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 70, sunset: 78, stargazing: 35,
    accessAlert: { message: "Uphill fire trail accessed from Dwight Way near Clark Kerr Campus. Moderate effort, ~20 min to ridge views.", type: 'hike' },
  },
  {
    id: "sf-huckleberry-preserve", name: "Huckleberry Botanic Regional Preserve",
    lat: 37.8510, lng: -122.1980,
    city: 'sf',
    category: 'park', elevation: 400,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 60, sunset: 40, stargazing: 68,
    accessAlert: { message: "Parking on Skyline Blvd. 1.7-mile loop trail. No dogs allowed. Mostly shaded — sunrise views from upper loop only.", type: 'hike' },
  },
  // — Lafayette —
  {
    id: "sf-lafayette-reservoir", name: "Lafayette Reservoir — Rim Trail",
    lat: 37.8850, lng: -122.1380,
    city: 'sf',
    category: 'hill', elevation: 280,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 78, sunset: 72, stargazing: 55,
    accessAlert: { message: "$7 parking fee. Paved Lakeside Trail is flat and easy; the unpaved Rim Trail (4.7 mi) with the views is steep. Gate hours vary seasonally.", type: 'paid' },
  },
  {
    id: "sf-briones-crest", name: "Briones Regional Park — Briones Crest",
    lat: 37.9250, lng: -122.1200,
    city: 'sf',
    category: 'hill', elevation: 470,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 82, sunset: 80, stargazing: 78,
    accessAlert: { message: "Multiple staging areas. Briones Crest requires a 2-3 mile hike from Alhambra Valley or Bear Creek staging. Park open 5 AM to 10 PM.", type: 'hike' },
  },
  {
    id: "sf-springhill-sunrise-trail", name: "Springhill Road Trailhead (Sunrise Trail)",
    lat: 37.9100, lng: -122.1350,
    city: 'sf',
    category: 'hill', elevation: 350,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 85, sunset: 70, stargazing: 65,
    accessAlert: { message: "Park at end of Springhill Road. Steep climb via Buckeye Ranch Trail to Sunrise Trail (~2.5 mi). No facilities.", type: 'hike' },
  },
  // — Moraga —
  {
    id: "sf-mulholland-ridge", name: "Mulholland Ridge",
    lat: 37.8480, lng: -122.1600,
    city: 'sf',
    category: 'hill', elevation: 350,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 75, sunset: 72, stargazing: 65,
    accessAlert: { message: "Access from Rheem Valley area. Old fire road, partially overgrown. Benches and tables at the top.", type: 'hike' },
  },
  {
    id: "sf-st-marys-peak", name: "St. Mary's Peak",
    lat: 37.8350, lng: -122.1350,
    city: 'sf',
    category: 'hill', elevation: 365,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 78, sunset: 75, stargazing: 72,
    accessAlert: { message: "5.4-mile moderate hike from Rancho Laguna Park in Moraga. Several creek crossings — can be muddy after rain.", type: 'hike' },
  },
  {
    id: "sf-kings-canyon-reservoir", name: "Kings Canyon / Upper San Leandro Reservoir",
    lat: 37.8300, lng: -122.1550,
    city: 'sf',
    // TODO: confirm category — inland reservoir on wooded ridge trails,
    // doesn't cleanly fit "waterfront" (not flat/urban) or "hill" (view is
    // over water, not open horizon).
    category: 'waterfront', elevation: 200,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 80, sunset: 55, stargazing: 60,
    accessAlert: { message: "EBMUD trail permit required ($3/year). Access from Valle Vista staging area. 6.7-mile loop.", type: 'paid' },
  },
  // — Orinda —
  {
    id: "sf-san-pablo-overlook", name: "San Pablo Reservoir Overlook",
    lat: 37.9050, lng: -122.2100,
    city: 'sf',
    // TODO: confirm category — inland reservoir viewpoint on Bear Creek Rd,
    // elevated but not coastal; ambiguous between "waterfront" and "hill".
    category: 'waterfront', elevation: 180,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 72, sunset: 78, stargazing: 55,
    accessAlert: { message: "Take Bear Creek Rd from Camino Pablo. EBMUD permit required for extended trails. Benches at high point.", type: 'paid' },
  },
  {
    id: "sf-orinda-oaks", name: "Orinda Oaks Open Space",
    lat: 37.8530, lng: -122.1730,
    city: 'sf',
    category: 'park', elevation: 290,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 55, sunset: 65, stargazing: 58,
  },
  {
    id: "sf-de-laveaga-trail", name: "De Laveaga Trail / Siesta Valley",
    lat: 37.8680, lng: -122.2150,
    city: 'sf',
    category: 'hill', elevation: 400,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 70, sunset: 80, stargazing: 62,
    accessAlert: { message: "EBMUD permit required. Access from Grizzly Peak Blvd or Scott's Peak trailhead. Adjacent to Cal Shakes theater.", type: 'paid' },
  },
  {
    id: "sf-tilden-inspiration-point", name: "Inspiration Point",
    lat: 37.8950, lng: -122.2250,
    city: 'sf',
    category: 'hill', elevation: 315,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 78, sunset: 82, stargazing: 70,
    accessAlert: { message: "EBMUD permit required. ~2-mile climb from Old San Pablo Trail junction. Fire roads, moderate effort.", type: 'paid' },
  },
  // — Danville —
  {
    id: "sf-las-trampas-ridge", name: "Las Trampas Ridge Trail",
    lat: 37.8300, lng: -122.0550,
    city: 'sf',
    category: 'hill', elevation: 520,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 80, sunset: 88, stargazing: 80,
    accessAlert: { message: "Access from Bollinger Canyon Rd staging area in San Ramon. Steep 2+ mile climb to the ridge. No shade.", type: 'hike' },
  },
  {
    id: "sf-rocky-ridge", name: "Rocky Ridge (Las Trampas)",
    lat: 37.8200, lng: -122.0480,
    city: 'sf',
    category: 'hill', elevation: 625,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 85, sunset: 90, stargazing: 85,
    accessAlert: { message: "Strenuous hike from Bollinger staging area. ~3 miles one way with 1,500+ ft gain. Bring plenty of water — exposed the entire way.", type: 'hike' },
  },
  {
    id: "sf-diablo-vista-park", name: "Diablo Vista Park",
    lat: 37.8130, lng: -121.9700,
    city: 'sf',
    category: 'park', elevation: 150,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 65, sunset: 70, stargazing: 35,
  },
  {
    id: "sf-sycamore-valley", name: "Sycamore Valley Park / Open Space",
    lat: 37.8200, lng: -122.0150,
    city: 'sf',
    category: 'park', elevation: 120,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 50, sunset: 55, stargazing: 30,
  },
  // — San Ramon —
  {
    id: "sf-bishop-ranch", name: "Bishop Ranch Regional Open Space",
    lat: 37.7650, lng: -122.0650,
    city: 'sf',
    category: 'hill', elevation: 340,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 72, sunset: 82, stargazing: 65,
    accessAlert: { message: "Walk-in access from Morgan Drive. Steep grades on fire trails. No facilities. Limited street parking.", type: 'hike' },
  },
  {
    id: "sf-tassajara-ridge", name: "Tassajara Ridge Trail",
    lat: 37.7400, lng: -121.9050,
    city: 'sf',
    category: 'hill', elevation: 280,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 68, sunset: 75, stargazing: 45,
    accessAlert: { message: "Tassajara Ridge Staging Area off Highland Road. Moderate climb. No shade — exposed ridgeline. Cattle may be on trail.", type: 'hike' },
  },
  {
    id: "sf-dougherty-valley-ridge", name: "Dougherty Valley Ridge Trail",
    lat: 37.7550, lng: -121.8950,
    city: 'sf',
    category: 'hill', elevation: 250,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 62, sunset: 72, stargazing: 40,
  },
  {
    id: "sf-old-ranch-park", name: "Old Ranch Park",
    lat: 37.7680, lng: -121.9600,
    city: 'sf',
    category: 'park', elevation: 120,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 45, sunset: 68, stargazing: 25,
  },
  // — South / Peninsula —
  {
    id: "sf-mori-point", name: "Mori Point",
    lat: 37.6180, lng: -122.4940,
    city: 'sf',
    category: 'coastal-bluff', elevation: 30,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 35, sunset: 92, stargazing: 72,
  },
  {
    id: "sf-pigeon-point", name: "Pigeon Point Lighthouse",
    lat: 37.1820, lng: -122.3930,
    city: 'sf',
    category: 'coastal-bluff', elevation: 10,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 40, sunset: 90, stargazing: 96,
  },
  {
    id: "sf-rancho-corral", name: "Rancho Corral de Tierra",
    lat: 37.5290, lng: -122.4560,
    city: 'sf',
    category: 'park', elevation: 120,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 55, sunset: 78, stargazing: 88,
    accessAlert: {
      message: "30 min south of SF in Montara. Open sunrise to sunset — no night access, so plan stargazing for winter when dark comes early.",
      type: 'hours',
    },
  },
  // — Hidden SF spots (id 104+) —
  // These are mostly access-gated: hike-only beaches, tide-dependent coves,
  // and a couple of urban surprises. Several carry an `accessAlert` because
  // showing up unprepared (wrong tide, wrong hours, no shoes) genuinely
  // wrecks the visit.
  {
    id: "sf-marshalls-beach", name: "Marshall's Beach",
    lat: 37.8030, lng: -122.4810,
    city: 'sf',
    category: 'beach', elevation: 5,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 30, sunset: 85, stargazing: 45,
    accessAlert: {
      message: "Hike-only access via Batteries to Bluffs trail. ~15 min steep descent, 470 stairs. No restrooms or water on the beach.",
      type: 'hike',
    },
  },
  {
    id: "sf-china-beach", name: "China Beach",
    lat: 37.7880, lng: -122.4910,
    city: 'sf',
    category: 'beach', elevation: 5,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 30, sunset: 80, stargazing: 35,
    accessAlert: {
      message: "Small cove — can get fully submerged at high tide. Check tide tables before going.",
      type: 'tide',
    },
  },
  {
    id: "sf-mullen-peralta", name: "Mullen Peralta Park",
    lat: 37.7390, lng: -122.4210,
    city: 'sf',
    category: 'hill', elevation: 110,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 40, sunset: 70, stargazing: 30,
  },
  {
    id: "sf-golden-gate-overlook", name: "Golden Gate Overlook (Batteries to Bluffs)",
    lat: 37.8060, lng: -122.4770,
    city: 'sf',
    category: 'coastal-bluff', elevation: 60,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 35, sunset: 90, stargazing: 40,
  },
  {
    id: "sf-fort-point", name: "Fort Point",
    lat: 37.8107, lng: -122.4770,
    city: 'sf',
    category: 'waterfront', elevation: 5,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 35, sunset: 75, stargazing: 25,
    accessAlert: {
      message: "Rooftop access is intermittent — check NPS site for current hours. Main area closes at 5 PM most days.",
      type: 'hours',
    },
  },
  {
    id: "sf-lands-end-labyrinth", name: "Lands End Labyrinth",
    lat: 37.7878, lng: -122.5050,
    city: 'sf',
    category: 'coastal-bluff', elevation: 30,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 30, sunset: 82, stargazing: 40,
    accessAlert: {
      message: "Off the main Lands End trail on an unmarked spur path. Look for the turnoff about 0.3 mi from the Merrie Way parking lot. Can be slippery when wet.",
      type: 'hike',
    },
  },
  {
    id: "sf-mile-rock-beach", name: "Mile Rock Beach",
    lat: 37.7870, lng: -122.5070,
    city: 'sf',
    category: 'beach', elevation: 3,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 25, sunset: 78, stargazing: 45,
    accessAlert: {
      message: "Requires a steep scramble down from Lands End trail. Only accessible at low tide — beach disappears at high tide.",
      type: 'tide',
    },
  },
  {
    id: "sf-palace-fine-arts", name: "Palace of Fine Arts",
    lat: 37.8020, lng: -122.4485,
    city: 'sf',
    category: 'park', elevation: 5,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 30, sunset: 65, stargazing: 15,
  },
  {
    id: "sf-pier-7", name: "Pier 7",
    lat: 37.7980, lng: -122.3980,
    city: 'sf',
    category: 'waterfront', elevation: 3,
    lightPollution: 'High', horizonQuality: 'Open',
    sunrise: 70, sunset: 45, stargazing: 10,
  },
  // — Rooftop / elevated venues —
  // High-elevation entries with paid-access alerts. Light pollution is High
  // (downtown), so stargazing scores stay low even though horizons are open.
  {
    id: "sf-cityscape-sky-bar", name: "Cityscape Sky Bar",
    lat: 37.7860, lng: -122.4100,
    city: 'sf',
    category: 'skyscraper', elevation: 140,
    lightPollution: 'High', horizonQuality: 'Open',
    sunrise: 50, sunset: 70, stargazing: 10,
    accessAlert: {
      message: "46th floor of Hilton Union Square. Bar — drinks required, no cover. Open 4 PM–midnight daily. 21+ after 10 PM.",
      type: 'paid',
    },
  },
  {
    id: "sf-cavana-rooftop", name: "Cavaña Rooftop",
    lat: 37.7710, lng: -122.3910,
    city: 'sf',
    category: 'skyscraper', elevation: 55,
    lightPollution: 'High', horizonQuality: 'Open',
    sunrise: 55, sunset: 68, stargazing: 8,
    accessAlert: {
      message: "17th floor of LUMA Hotel, Mission Bay. Open-air bar, no reservations for small parties. Cocktails $15–22.",
      type: 'paid',
    },
  },
  // — Greater Bay Area niche picks (id 115+) —
  // Bolinas Ridge (92) and Rancho Corral de Tierra (103) already cover two
  // of the marquee regional picks; their alerts were added in place above.
  {
    id: "sf-point-bonita", name: "Point Bonita Lighthouse",
    lat: 37.8157, lng: -122.5297,
    city: 'sf',
    category: 'coastal-bluff', elevation: 40,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 35, sunset: 85, stargazing: 70,
    accessAlert: {
      message: "Trail to lighthouse open Sat–Mon, 12:30–3:30 PM only. Suspension bridge crossing required — not for those with vertigo.",
      type: 'hours',
    },
  },
  {
    id: "sf-kirby-cove", name: "Kirby Cove",
    lat: 37.8260, lng: -122.4970,
    city: 'sf',
    category: 'beach', elevation: 5,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 30, sunset: 88, stargazing: 65,
    accessAlert: {
      message: "Permit required for camping. Day-use involves a steep 1-mile trail down from Conzelman Rd. Gate closes at sunset unless camping.",
      type: 'hike',
    },
  },
  // — North Beach / Telegraph Hill niche spots —
  // Tight cluster on and around Telegraph Hill. High light pollution downtown
  // keeps stargazing scores low even where the horizon opens up. Ina Coolbrith
  // Park and Pioneer Park are intentionally NOT re-added here — they already
  // exist above (ids sf-ina-coolbrith and sf-coit-tower / "Coit Tower /
  // Pioneer Park"). "Pretty View" is pending real coordinates and is omitted
  // until confirmed.
  {
    id: "sf-jack-early-park", name: "Jack Early Park",
    lat: 37.8040, lng: -122.4055,
    city: 'sf',
    category: 'hill', elevation: 60,
    lightPollution: 'High', horizonQuality: 'Open',
    sunrise: 65, sunset: 55, stargazing: 8,
    accessAlert: {
      message: "Tiny park on north slope of Telegraph Hill. ~60 steps up via hidden stairway off Francisco St. Gated, closes at night.",
      type: 'hours',
    },
  },
  {
    id: "sf-filbert-steps", name: "Filbert Steps / Grace Marchant Garden",
    lat: 37.8023, lng: -122.4045,
    city: 'sf',
    category: 'park', elevation: 50,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 55, sunset: 50, stargazing: 5,
  },
  {
    id: "sf-greenwich-steps", name: "Greenwich Steps",
    lat: 37.8030, lng: -122.4060,
    city: 'sf',
    category: 'park', elevation: 55,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 52, sunset: 48, stargazing: 5,
  },
  {
    id: "sf-vallejo-steps", name: "Vallejo Street Steps",
    lat: 37.7975, lng: -122.4115,
    city: 'sf',
    category: 'park', elevation: 55,
    lightPollution: 'High', horizonQuality: 'Partial',
    sunrise: 48, sunset: 52, stargazing: 8,
  },
  // — South Bay (prefix sb-) —
  // Grouped under city 'sf' so they surface on the SF map; the dynamic pan
  // bounds in MapView (boundsFromSpots) auto-extend south to Mt. Umunhum
  // (37.16) and east to Mt. Hamilton / Lick Observatory (-121.64). Weather
  // mode uses its own fixed SF bounds and is unaffected.
  // — Hilltop / ridge viewpoints —
  {
    id: "sb-mount-umunhum", name: "Mount Umunhum Summit",
    lat: 37.1605, lng: -121.8986,
    city: 'sf',
    category: 'hill', elevation: 1063,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 70, sunset: 88, stargazing: 92,
    accessAlert: {
      message: "3.7-mile one-way climb from Bald Mountain lot. Summit closes 30 min after sunset. No water, no dogs.",
      type: 'hike',
    },
  },
  {
    id: "sb-mt-hamilton", name: "Mt. Hamilton / Lick Observatory",
    lat: 37.3414, lng: -121.6429,
    city: 'sf',
    category: 'hill', elevation: 1283,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 85, sunset: 80, stargazing: 98,
    accessAlert: {
      message: "1 hr winding drive from San Jose via Mt Hamilton Rd. Observatory visitor hours limited, check schedule. Road can close in winter.",
      type: 'hours',
    },
  },
  {
    id: "sb-communications-hill", name: "Communications Hill / Vieira Park",
    lat: 37.2980, lng: -121.8550,
    city: 'sf',
    category: 'hill', elevation: 120,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 60, sunset: 72, stargazing: 20,
    accessAlert: {
      message: "Grand Staircase (250 steps) from base. Residential area, keep noise down.",
      type: 'info',
    },
  },
  {
    id: "sb-monument-peak", name: "Monument Peak (Ed Levin)",
    lat: 37.4666, lng: -121.8569,
    city: 'sf',
    category: 'hill', elevation: 730,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 78, sunset: 82, stargazing: 70,
    accessAlert: {
      message: "$6 vehicle entry. Steep 4+ mile hike from Sandy Wool Lake. Park open 8 AM to sunset.",
      type: 'hike',
    },
  },
  {
    id: "sb-sierra-vista", name: "Sierra Vista Open Space",
    lat: 37.2230, lng: -121.7980,
    city: 'sf',
    category: 'hill', elevation: 400,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 65, sunset: 75, stargazing: 65,
  },
  {
    id: "sb-black-mountain", name: "Black Mountain (Monte Bello)",
    lat: 37.3210, lng: -122.0980,
    city: 'sf',
    category: 'hill', elevation: 825,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 72, sunset: 80, stargazing: 72,
    accessAlert: {
      message: "Strenuous 8+ mile round trip from Rancho San Antonio. Fully exposed, bring water.",
      type: 'hike',
    },
  },
  // — Parks / preserves with views —
  {
    id: "sb-rancho-san-antonio", name: "Rancho San Antonio",
    lat: 37.3370, lng: -122.0890,
    city: 'sf',
    category: 'park', elevation: 120,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 55, sunset: 68, stargazing: 45,
    accessAlert: {
      message: "Open 30 min before sunrise to 30 min after sunset. Parking fills early on weekends.",
      type: 'hours',
    },
  },
  {
    id: "sb-stanford-dish", name: "Stanford Dish",
    lat: 37.4060, lng: -122.1755,
    city: 'sf',
    category: 'park', elevation: 180,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 58, sunset: 72, stargazing: 25,
    accessAlert: {
      message: "Open sunrise to sunset only (hours vary seasonally). No dogs, no bikes. Stanford security on-site.",
      type: 'hours',
    },
  },
  {
    id: "sb-fremont-older", name: "Fremont Older Open Space",
    lat: 37.2890, lng: -122.0780,
    city: 'sf',
    category: 'park', elevation: 260,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 55, sunset: 70, stargazing: 50,
    accessAlert: {
      message: "Open sunrise to 30 min after sunset.",
      type: 'hours',
    },
  },
  {
    id: "sb-ed-levin", name: "Ed Levin County Park",
    lat: 37.4580, lng: -121.8530,
    city: 'sf',
    category: 'park', elevation: 150,
    lightPollution: 'Low', horizonQuality: 'Partial',
    sunrise: 50, sunset: 62, stargazing: 42,
    accessAlert: {
      message: "$6 vehicle entry. Open 8 AM to sunset.",
      type: 'paid',
    },
  },
  // — Waterfront / flat horizon —
  {
    id: "sb-alviso-marina", name: "Alviso Marina County Park",
    lat: 37.4270, lng: -121.9760,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 70, sunset: 78, stargazing: 28,
  },
  {
    id: "sb-shoreline-park", name: "Shoreline at Mountain View",
    lat: 37.4310, lng: -122.0880,
    city: 'sf',
    category: 'waterfront', elevation: 3,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 55, sunset: 62, stargazing: 15,
  },
  {
    id: "sb-baylands-sunnyvale", name: "Sunnyvale Baylands Park",
    lat: 37.4170, lng: -122.0320,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 60, sunset: 65, stargazing: 18,
  },
  {
    id: "sb-coyote-point", name: "Coyote Point Recreation Area",
    lat: 37.5894, lng: -122.3261,
    city: 'sf',
    category: 'waterfront', elevation: 15,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 62, sunset: 68, stargazing: 15,
    accessAlert: {
      message: "$6 vehicle entry fee.",
      type: 'paid',
    },
  },
  // — Sausalito (Marin) —
  // Grouped under city 'sf' like the other Marin Headlands entries (Battery
  // Spencer, Hawk Hill, Kirby Cove, Point Bonita) — the SF map's static
  // bounds reach north of the Golden Gate to cover them. The town waterfront
  // faces east across the bay, so those spots earn high sunrise scores; the
  // real sunset payoff is from the ridge and headlands trails just west of
  // town, where the western horizon opens up over the Pacific hills.
  {
    id: "sf-wolfback-ridge", name: "Wolfback Ridge",
    lat: 37.8513, lng: -122.4970,
    city: 'sf',
    category: 'hill', elevation: 230,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 60, sunset: 90, stargazing: 68,
    accessAlert: {
      message: "Ridge trail above Sausalito off Alta/Spencer Ave. Open western horizon over the headlands. ~1 mi climb on fire road, fully exposed — bring a layer for the wind.",
      type: 'hike',
    },
  },
  {
    id: "sf-morning-sun-trail", name: "Morning Sun Trail Overlook",
    lat: 37.8455, lng: -122.4920,
    city: 'sf',
    category: 'hill', elevation: 200,
    lightPollution: 'Low', horizonQuality: 'Open',
    sunrise: 62, sunset: 88, stargazing: 65,
    accessAlert: {
      message: "Steep climb from the Spencer Ave trailhead off Hwy 101 up to the SCA Trail. ~0.5 mi but stair-like — sunset views over the Marin Headlands at the top.",
      type: 'hike',
    },
  },
  {
    id: "sf-hurricane-gulch", name: "Hurricane Gulch Overlook",
    lat: 37.8478, lng: -122.4832,
    city: 'sf',
    category: 'hill', elevation: 110,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 65, sunset: 82, stargazing: 35,
  },
  {
    id: "sf-cavallo-point", name: "Cavallo Point / Fort Baker",
    lat: 37.8338, lng: -122.4806,
    city: 'sf',
    category: 'waterfront', elevation: 8,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 80, sunset: 72, stargazing: 30,
  },
  {
    id: "sf-sausalito-boardwalk", name: "Sausalito Boardwalk (Bridgeway)",
    lat: 37.8569, lng: -122.4789,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 82, sunset: 60, stargazing: 22,
  },
  {
    id: "sf-gabrielson-park", name: "Gabrielson Park",
    lat: 37.8567, lng: -122.4793,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 80, sunset: 58, stargazing: 20,
  },
  {
    id: "sf-dunphy-park", name: "Dunphy Park",
    lat: 37.8639, lng: -122.4838,
    city: 'sf',
    category: 'waterfront', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Partial',
    sunrise: 78, sunset: 56, stargazing: 22,
  },
  {
    id: "sf-schoonmaker-beach", name: "Schoonmaker Point Beach",
    lat: 37.8662, lng: -122.4895,
    city: 'sf',
    category: 'beach', elevation: 2,
    lightPollution: 'Mid', horizonQuality: 'Open',
    sunrise: 80, sunset: 62, stargazing: 25,
  },
];

export function getConditionLabel(score: number): string {
  if (score >= 80) return 'Radiant';
  if (score >= 60) return 'Clear';
  if (score >= 40) return 'Subdued';
  return 'Obscured';
}

export function getPoetic(type: 'now' | 'sunrise' | 'sunset' | 'stargazing', score: number): string {
  if (type === 'now') {
    if (score >= 80) return 'Clear skies & golden light';
    if (score >= 60) return 'Bright with a few clouds drifting';
    if (score >= 40) return 'Hazy with patches of blue';
    return 'Gray & socked in';
  }
  if (type === 'sunrise') {
    if (score >= 80) return 'Shades of coral, gold, & rose';
    if (score >= 60) return 'Soft pinks & warming amber';
    if (score >= 40) return 'Gentle peach & muted lilac';
    return 'Pale gray with hints of blush';
  }
  if (type === 'sunset') {
    if (score >= 80) return 'Shades of scarlet, orange, & lavender';
    if (score >= 60) return 'Warm amber & dusky violet';
    if (score >= 40) return 'Muted rose & fading copper';
    return 'Overcast gray with dim ochre';
  }
  if (score >= 80) return 'Deep indigo & scattered starlight';
  if (score >= 60) return 'Navy skies with visible constellations';
  if (score >= 40) return 'Dim stars behind city haze';
  return 'Washed out by ambient light';
}
