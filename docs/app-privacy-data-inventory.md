...
| Base map style, glyph, sprite, and vector-tile requests | Visible tile coordinates, zoom, viewport-derived request sequence, HTTP referrer, browser and operating-system information, timestamps, and network information that can include an IP address at the delivery layer | OpenFreeMap and its optional Cloudflare CDN | Render the map | Ordinary browser or WebKit network caching may apply. Soleil does not service-worker-precache or advertise offline map data | OpenFreeMap says normal origin logs omit IP addresses, anonymized request logs may be retained indefinitely, and incident-response IP logging may be enabled for up to 30 days. Cloudflare may independently process request and routing metadata | Tile requests reveal the viewed area. OpenFreeMap permits commercial websites and apps without a key, but its service has no SLA and its current terms contain an under-18 ambiguity. Written clarification or an approved alternate provider remains a public App Store release gate. |
...
For Open-Meteo, OpenFreeMap, Cloudflare, Google Maps, Vercel, Apple diagnostics, and any provider added before submission, record:
